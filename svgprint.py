...
<svg
  ...
  <g
    ...
    <path
       style="fill:#00c000;fill-opacity:0.43378996;stroke:#000000;stroke-width:0.79987603px;stroke-linecap:butt;stroke-linejoin:miter;stroke-opacity:1"
...
def im_output(value):
	...
	for f in files:
		if 'imageSVG' in f:
			if int(f.replace('imageSVG','').replace('.svg','')) > i:
				i = int(f.replace('imageSVG','').replace('.svg',''))
	i += 1

	open('imageSVG' + str(i) + '.svg', 'w').write(outSTR.replace('MYHEIGHTVALUE', str(value * 320)))
